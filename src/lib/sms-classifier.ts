export type SmsType = 'otp' | 'offer' | 'recharge' | 'balance' | 'transfer' | 'info'
...
export function classifySms(sender: string, content: string): SmsType {
  ...
  if (c.includes('solde') || c.includes('balance')) return 'balance'
  if (c.includes('recharge') || c.includes('recharg') || c.includes('credit') || c.includes('top-up')) return 'recharge'
